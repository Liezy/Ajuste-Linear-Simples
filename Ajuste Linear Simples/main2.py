...
# Tabela com os valores calculados, arredondados para duas casas decimais
table_data = []
for i in range(n):
    table_data.append([
        i + 1,
        ...
        round(y[i], 2),
        round(y_best[i], 2),
        round(abs(y[i] - y_best[i]), 2),
        round(y_aleat1[i], 2),
        round(abs(y[i] - y_aleat1[i]), 2),
        round(y_aleat2[i], 2),
        round(abs(y[i] - y_aleat2[i]), 2)
    ])

# Adicionando a linha final com os desvios totais
table_data.append([
    "D",
    "",
    "",
...
# Exibindo a tabela
columns = ["i", "xi", "yi", "ŷ (melhor reta)", "di (melhor reta)", "ỹ (reta aleatória 1)", "di (reta aleatória 1)", "ȳ (reta aleatória 2)", "di (reta aleatória 2)"]
